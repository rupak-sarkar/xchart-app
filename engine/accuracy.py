"""Backtest v7.0 — σ-based neutral band, 5% SL, cap-aware horizons."""
import numpy as np
import pandas as pd
...
MIN_HOLD_DAYS = 7  # Minimum before flip allowed
...
def _compute_sigma_threshold(returns, forward_days):
    """Dynamic neutral band = rolling 20d std dev × √forward_days.
    
    This naturally adapts:
    - Volatile stocks get wider bands → fewer false signals
    - Stable stocks get tighter bands → more signals captured
    - Longer horizons get wider bands → appropriate for the timeframe
    """
    if len(returns) < 25:
        return 1.5  # Default fallback
    
    # Rolling 20-day std dev of daily returns
    ...
    # Annualize daily std, then scale to horizon
    # threshold = daily_std × √(forward_days) × 100 (as percentage)
    threshold = recent_std * np.sqrt(forward_days) * 100
    
    # Clamp between 1.5% and 8%
    return max(1.5, min(8.0, threshold))
...
def compute_per_ticker_accuracy(stock_df, mcap_threshold):
    ...
    if results:
        ...
        print(f"  {'Cat':<10} {'N':>3}  {'DirAcc':>6}  {'Strict':>6}  {'σ-Thresh':>8}  {'SL':>6}  {'Fwd':>4}")
        print(f"  {'-'*52}")
        for c in ['MEGA', 'LARGE', 'MID', 'SMALL']:
            ...
            avg_strict = np.mean([100 - r['BT_6M'] for r in items])  # placeholder
            avg_thresh = np.mean([r['BT_Threshold'] for r in items])
            fwd = HORIZONS[c]
            print(f"  {c:<10} {n:>3}  {avg_acc:>5.1f}%  "
                  f"{'':>6}  ±{avg_thresh:>5.2f}%  {STOP_LOSS_PCT:>5.1f}%  {fwd:>3}d")
        ...
        for c in ['MEGA', 'LARGE', 'MID', 'SMALL']:
            ...
            wr = np.mean([r['TR_win_rate'] for r in items if r['TR_total_trades'] > 0])
            aw = np.mean([r['TR_avg_win_pct'] for r in items if r['TR_total_trades'] > 0])
            al = np.mean([r['TR_avg_loss_pct'] for r in items if r['TR_total_trades'] > 0])
            pf = np.mean([r['TR_profit_factor'] for r in items if r['TR_total_trades'] >= 5])
            tr = np.mean([r['TR_total_return_pct'] for r in items if r['TR_total_trades'] > 0])
            hd = np.mean([r['TR_avg_holding_days'] for r in items if r['TR_total_trades'] > 0])
            sl = sum(r['TR_stop_loss_exits'] for r in items)
            slp = sl / tt * 100 if tt > 0 else 0
            print(f"  {c:<10}{tt:>6} {wr:>5.1f}% {aw:>6.2f}% {al:>6.2f}% {pf:>5.2f} {tr:>+7.1f}% {hd:>5.1f}d {slp:>4.1f}%")
...
def print_accuracy_report(bt_results, scored, hdf, all_rows):
    ...
    print(f"\n{'='*110}")
    print(f"PREDICTION + TRADE REPORT (v7.0 σ-band + 5% SL)")
    print(f"{'='*110}")
    ...
    for c in ['MEGA', 'LARGE', 'MID', 'SMALL']:
        ...
        avg_acc = np.mean([r['BT_6M'] for r in items])
        avg_thresh = np.mean([r['BT_Threshold'] for r in items])
        print(f"  {c:<10} {avg_acc:>5.1f}%  ±{avg_thresh:>5.2f}%  {HORIZONS[c]:>5}d  {HOLD_DAYS[c]:>5}d  {len(items):>4}")
    ...
    if hdf is not None and not hdf.empty and 'Date' in hdf.columns:
        ...
        for i in range(len(dates) - 1):
            ...
            for _, mr in merged.iterrows():
                dir1 = int(mr.get('Composite_Direction_1', 0))
