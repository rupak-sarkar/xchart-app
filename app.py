...
from engine.config import (
    TODAY_IST, NEWS_START_DATE, NEWS_CUTOFF_TIME, DATA_FILE,
...
def run():
    ...
    hd = 0; td2 = 0; total_catalyst = 0; total_noise_filtered = 0
    ...
    for tk in tl:
        ...
        base = {"Ticker": tk, "Sector": sm.get(tk, ""), "Actual_Direction": ad, "Actual_Return_Pct": ret}

        if cls == "no_news":
            nonews.append({**base, "Latest_Headline": "", "News_Source": "", "News_Time": "", "News_URL": "",
        ...
        if cls == "filing_only":
            ...
            filing.append({**base, "Latest_Headline": p.get("headline", ""), "News_Source": "NSE Official",
        ...
        scored.append({**base, "Latest_Headline": pe["headline"], "News_Source": " | ".join(usrc),
                       "News_Time": pt.replace(",", "") if pt else "", "News_URL": pu,
                       "Headline_Count": len(entries), "Forecast_Score": score,
                       "Forecast_Direction": direction, "Severity": sev, "Impact": imp, "Signal_Quality": q})
    ...
    for row in all_rows:
        ...
        row["_sector"] = sector
        if sector: all_sectors.add(sector); sc_count += 1
