...
def phase2_finbert(tl, sm, nc):
    ...
    for tk in tl:
        ...
        if cls == "filing_only":
            ...
            filing.append({
                **base,
                "Latest_Headline": p.get("headline", ""),
                "News_Source": "NSE Official",
                "News_Time": p.get("pub_time", "").replace(",", ""),
                "News_URL": nu,
                "Headline_Count": len(fe),
                "Forecast_Score": 0.0,
                "Forecast_Direction": 0,
                "Severity": "Filing Only",
                "Impact": "",
                "Streak_Days": 0,
                "Streak_Return": 0.0,
                "Momentum": "",
        ...
        tag = f"[{cat_count}cat/{len(entries)}h]"
        print(
            f"[{len(scored)+1:3d}] {tk:<14s} {DM.get(direction,'?'):4s} "
            f"Score:{score:+6.1f} Ret:{ret:+6.2f}% "
            f"{'HIT' if hit else 'MISS'} {tag}"
        )
        scored.append({
            **base,
            "Latest_Headline": pe["headline"],
...
def phase_composite(scored, filing, nonews, regime):
    ...
    if "BEAR" in regime["regime"]:
        if regime["regime"] == "BEAR":
            rdesc = "BULL dampened 60%"
        else:
            rdesc = "BULL dampened 80%"
    elif "BULL" in regime["regime"]:
        if regime["regime"] == "BULL":
            rdesc = "BEAR dampened 60%"
        else:
            rdesc = "BEAR dampened 80%"
    else:
        rdesc = "no adjustment"
    ...
    if t_total > 0:
        pct = t_hit / t_total * 100
        print(
            f"\n  Tech-only: Bull:{t_bull} Bear:{t_bear} Neut:{t_neut} "
            f"| Hit:{t_hit}/{t_total} = {pct:.1f}%"
        )
